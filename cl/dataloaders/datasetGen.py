...
def PermutedGen(train_dataset, val_dataset, n_permute, first_split_sz=2, other_split_sz=2, rand_split=False, remap_class=False):
    ...
    for p in range(n_permute):
        for name, class_list in class_lists.items():
            ...
            shuffle(rand_ind)
            train_dataset_splits[name] = AppendName(Permutation(train_dataset,rand_ind, class_list, remap_class), name)
            val_dataset_splits[name] = AppendName(Permutation(val_dataset,rand_ind, class_list, remap_class), name)
            task_output_space[name] = len(class_list)
